...
int processHostCommand(void *b,uint32_t ch_num)
{
	...
	switch (cmd_id)
	{
		...
		case CMD_ENC_OFFSET:
			...
			#ifndef RANISHOW
			AbsEncOffset=AbsEncoderData.raw32Data>>3;
			#else
			AbsEncOffset=(uint32_t)((AbsEncoderData.raw32Data>>25)&0x3FFFFFF);
			#endif
			__restoreInterrupts(key);
			SetAbsEncOffset(&AbsEncOffset);
		...
		case CMD_TRAVEL_PIN:
			...
			#ifdef TRAVEL_PIN
			SetTravelPinCmd((char*)PAYLOAD_DATA_START(b));
			#endif
		...
		case CMD_GET_REOSTAT:
			...
			#ifdef TRAVEL_PIN
			memset(pr, 0, ((sizeof(char))*60));
			pr->length=GetTravelPinReostat(PAYLOAD_START(pr));
			resp_packet=makeSinglePacketResponse(&cmdResBuffers,pr,RESP_BUFFER_GET_TIMEOUT);
			#endif
